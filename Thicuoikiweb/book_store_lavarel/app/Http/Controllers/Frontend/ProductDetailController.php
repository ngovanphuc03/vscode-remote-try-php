...
class ProductDetailController extends FrontendController
{
	...
	function filterByLike($item, $value) {
		return stristr($item, $value) !== false;
	}
	...
	public function getListRatingProduct(Request $request, $slug)
	{
		...
		if ($id) {
			...
			$ratings = Rating::with('user:id,name')
				->where('r_product_id', $id);
			if ($number = $request->s) {
				$ratings->where('r_number', $number);
			}

			$ratings = $ratings->orderByDesc('id')
				->paginate(5);

			// if ($name = Str::slug($request->key_search, '')) {
			// 	$ratings = $ratings->filter(function ($value) use ($name) {
			// 		return str_contains(Str::slug($value->user->name, ''), $name);
			// 	});
			// };
